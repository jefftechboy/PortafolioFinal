...
# ---------------------------------------------------- EMPLEADOS  -------------------------------------------------
    # Para la pagina de agendaEmp de la empresa
        path("agendaEmp/", listar_agenda_emp, name= "agendaEmp"),
    # PERFIL EMPLEADO
    ...
    # CAMBIO DE CONTRASEÑA
        # Paso 1: ingresar correo
            path('password-reset/', auth_views.PasswordResetView.as_view(
                template_name='registration/cambioContrasena/formularioCambioContrasena.html',
                email_template_name='registration/cambioContrasena/correoCambioContrasena.html',
                subject_template_name='registration/cambioContrasena/password_reset_subject.txt',
                success_url='/password-reset/done/',), name='password-reset'),
        # Paso 2: confirmación de que se envió el correo
            path('password-reset/done/', auth_views.PasswordResetDoneView.as_view(
                template_name='registration/cambioContrasena/cambioContrasenaEnviado.html'), name='password_reset_done'),
        # Paso 3: ingresar nueva contraseña
            path('accounts/reset/<uidb64>/<token>/', auth_views.PasswordResetConfirmView.as_view(
            template_name='registration/cambioContrasena/actualizarContrasenas.html',
            success_url='/reset/done/'), name='password_reset_confirm'),
        # Paso 4: contraseña cambiada correctamente
            path('reset/done/', auth_views.PasswordResetCompleteView.as_view(
                template_name='registration/cambioContrasena/cambioContrasenaCompletado.html'), name='password_reset_complete'),
] 